...
class _ExampleAppState extends State<ExampleApp> {
  ...
  Future _loadFile() async {
    ...
    final file = File('${dir.path}/underwater.mp3');
  ...
  Widget localAsset() {
    return SingleChildScrollView(
      child: _Tab(children: [
        Text('Play Local Asset \'underwater.mp3\':'),
        _Btn(txt: 'Play', onPressed: () => audioCache.play('underwater.mp3')),
        Text('Loop Local Asset \'underwater.mp3\':'),
        _Btn(txt: 'Loop', onPressed: () => audioCache.loop('underwater.mp3')),
        Text('Play Local Asset \'rain.mp3\':'),
        _Btn(txt: 'Play', onPressed: () => audioCache.play('rain.mp3')),
        Text('Play Local Asset In Low Latency \'underwater.mp3\':'),
        _Btn(
            txt: 'Play',
            onPressed: () =>
                audioCache.play('underwater.mp3', mode: PlayerMode.LOW_LATENCY)),
        Text('Play Local Asset Concurrently In Low Latency \'underwater.mp3\':'),
        _Btn(
            txt: 'Play',
            onPressed: () async {
              await audioCache.play('underwater.mp3', mode: PlayerMode.LOW_LATENCY);
              await audioCache.play('rain.mp3', mode: PlayerMode.LOW_LATENCY);
            }),
        Text('Play Local Asset In Low Latency \'rain.mp3\':'),
        _Btn(
            txt: 'Play',
            onPressed: () =>
                audioCache.play('rain.mp3', mode: PlayerMode.LOW_LATENCY)),
  ...
  Future<int> _getDuration() async {
    File audiofile = await audioCache.load('rain.mp3');
      ...
      builder: (BuildContext context, AsyncSnapshot<int> snapshot) {
        switch (snapshot.connectionState) {
          ...
          case ConnectionState.done:
            ...
            return Text(
                'rain.mp3 duration is: ${Duration(milliseconds: snapshot.data)}');
        }
  ...
  Widget notification() {
    return _Tab(children: [
      Text('Play notification sound: \'ocean.mp3\':'),
      _Btn(
          txt: 'Play',
          onPressed: () =>
              audioCache.play('ocean.mp3', isNotification: true)),
    ]);
  }
